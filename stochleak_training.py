...
from module_stochastic_2 import StochParameters
from module_stochastic_2 import StochCell,StochMWCell
from module_dwmtj_lif import DWMTJParameters,DWMTJCell
...
target_dir = f'240610_stochmwleak_s{seeds}_fashion_noise{noise_std}_seqnet{hidden}_neurmw_ep{EPOCHS}_lr{LR}_T{T}_alpha{alpha}_beta{beta}_pf{f_poisson}_bn_initxunif'
...
class SeqNet(torch.nn.Module):
    def __init__(
        ...
        self.stoch0 = DWMTJCell(p=DWMTJParameters(method=method, alpha=alpha))
        self.stoch1 = DWMTJCell(p=DWMTJParameters(method=method, alpha=alpha))
        self.stoch2 = DWMTJCell(p=DWMTJParameters(method=method, alpha=alpha))
