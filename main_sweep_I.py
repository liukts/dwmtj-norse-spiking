...
if not os.path.isdir("./outputs/"):
    os.mkdir("./outputs/")
if not os.path.isdir("./outputs/" + target_dir1):
    os.mkdir("./outputs/" + target_dir1)
...
SEED = 5            # number of seeds to run the network (kept as 1 if manual seed is applied)
MTYPE = 'conv'      # snn type
...
I = np.linspace(60e-6, 120e-6, 7) 
#T = np.linspace(5,80,16)

np.save("./outputs/" + target_dir + "/I.npy", np.array(I))
...
for f in range(0,len(I)):
    ...
    while s < SEED:
        ...
        if MTYPE == 'rec':
            snn = RecNet(28*28, 100, 10, I= I[f])
        elif MTYPE == 'conv':
            snn = ConvNet(alpha=100,w2=25e-9, I= I[f])
...
fin_acc = np.concatenate(fin_acc).reshape(SEED, len(I), EPOCHS)
...
#Single Seed Graphing 
#legend = []
#for ii in range(0, len(I)):
#    legend.append(str(int((I[ii])/1e-6)) + "e-6")
#
